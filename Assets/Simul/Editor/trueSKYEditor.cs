...
namespace simul
{
	...
	public class trueSKYEditor : Editor
	{
		...
		[SerializeField]
		static bool windField = false;	
		
		[SerializeField]
		static bool cloudWindow = false;
		[SerializeField]
		static bool SDFDivisors = false;
		...
		string[] renderOptions = new string[]
			{
				"Grid", "Fixed", "Variable Grid", "Variable SDF"
            };
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.BeginVertical();
			{
				...
				if (general)
				{
					trueSky.MetresPerUnit = EditorGUILayout.Slider("Scale (Metres per Unit)", trueSky.MetresPerUnit, 0.0001f, 10.0f);
                ...
                EditorGUILayout.Space();
                windField = EditorGUILayout.Foldout(windField, "Wind", outerFoldoutStyle);
				if(windField)
				{
				...
				if (clouds)
				{
					...
					//if (render)
					{
						...
						else
						{
							...
							if (trueSky.SimulVersion < trueSky.MakeSimulVersion(4, 3))
							{
								...
								Debug.Log("Variable is only supported in trueSKY 4.3+");
							}
							else
							{
                    ...
                    EditorGUILayout.Space();
                    cloudWindow = EditorGUILayout.Foldout(cloudWindow, "Cloud Window", innerFoldoutStyle);
                    if (cloudWindow)
                    {
                        ...
                        trueSky.WindowWidthKm = EditorGUILayout.IntSlider("Window Width (Km)", trueSky.WindowWidthKm, 200, 2000);
                    ...
                    {
                        EditorGUILayout.Space();
                        SDFDivisors = EditorGUILayout.Foldout(SDFDivisors, "SDF Divisors", innerFoldoutStyle);
                        if (SDFDivisors)
                        {
				...
				if (celestial)
				{
					...
					if (stars)
					{
						...
						trueSky.BackgroundBrightness = EditorGUILayout.Slider("Cosmic Background Brightness", trueSky.BackgroundBrightness, 0.0f, 10.0f);
						trueSky.StarBrightness = EditorGUILayout.Slider("Star Brightness", trueSky.StarBrightness, 0.0f, 10.0f);
				...
				if (interpolation)
				{
					...
					if(trueSky.InterpolationMode == 1)
						trueSky.InterpolationGameTimeHours = EditorGUILayout.Slider("Game time hours per subdivision", trueSky.InterpolationGameTimeHours, 1, 24);
					if(trueSky.InterpolationMode == 2)
						trueSky.InterpolationRealTime = EditorGUILayout.Slider("Seconds per subdivision", trueSky.InterpolationRealTime, 1, 24);
